...
const CustomerInfo: React.FC = () => {
  ...
  const searchHandle = () => {
    ...
    Http.reqGetCustomer("/getCustomerInfo", { name, offset: 0, size: 10 }).then(
      (response: any) => {
  ...
  React.useEffect(() => {
    Http.reqGetCustomer("/getCustomerInfo", {
      name: "",
      offset: 0,
      size: 10,
    }).then((response: any) => {
  ...
  const onCreate = async (values: any) => {
    ...
    if (rowData["id"]) {
      let id = rowData["id"];
      await Http.reqDeitCustomer("/updateCustomerInfo", {
        registerDate,
        name,
        status,
        phone,
      ...
      await Http.reqAddCustomer("/insertCustomerInfo", {
        registerDate,
        name,
        status,
        phone,
