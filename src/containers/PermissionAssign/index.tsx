...
const PermissionAssign: React.FC = () => {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (_text: any, record: any, _index: any) => {
        return (
          <Button type="primary" onClick={() => authorityEdit(record)}>
            编辑权限
          </Button>
        );
      },
  ...
  const editAuthority = async (checkedKeys: any) => {
    ...
    await Http.reqEditAuthorityInfo("/editAuthorityInfo", {
      id,
      authorityInfo: checkedKeys.join(),
    });
    refreshTable();
    setIsModalVisible(false);
  };
  const refreshTable = (ifFirst?: boolean) => {
    Http.reqGetCustomer("/getUserInfo", {
      username: "",
      offset: 0,
      size: 10,
    }).then((response: any) => {
