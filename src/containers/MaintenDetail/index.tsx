...
const MaintenDetail: React.FC = () => {
  ...
  const refreshTable = async (ifFirst?: boolean) => {
    try {
      await Http.reqGetCustomer("/getMaintenDetail", {
        repairMan: "",
        offset: 0,
        size: 10,
      }).then((response: any) => {
    ...
    },
    {
      ...
      render: (_text: any, _record: any) => {
        ...
        return status === "费用审核" && problemDesc ? (
          ...
          <Button type="primary" onClick={() => handleServiceFee(_record)}>
            服务费用
          </Button>
        ) : status == "服务稽查" ? (
          <Button type="primary" onClick={() => handleServiceAudit(_record)}>
            服务稽查
          </Button>
        ) : (
          <></>
  ...
  return (
    <>
      ...
      />
      <ServiceAudit
        onAuditCreate={onAuditCreate}
        isAuditModalVisible={isAuditModalVisible}
        handleAuditCancel={handleAuditCancel}
        getAuditForm={getAuditForm}
      />
    </>
  );
};
