...
const StaffInfo: React.FC = () => {
  ...
  const onCreate = async(values: any) => {
    ...
    if(rowData["id"]){
      let id = rowData["id"]
      await Http.reqDeitCustomer("/updateStaffInfo",{hireDate,name,status,phone,id,username,password,role})
      
    }else{
      await Http.reqAddCustomer("/insertStaffInfo",{hireDate,name,status,phone,username,password,role})
    }
    setIsModalVisible(false)
    refreshTable()
  };
  React.useEffect(()=>{
    Http.reqGetCustomer("/getStaffInfo",{name:"",offset:0,size:10}).then((response:any)=>{
  ...
  return (
    <>
      ...
      <AddOrEditStaff ModalName={ModalName} onCreate={onCreate} isModalVisible={isModalVisible} handleCancel={handleCancel} getForm={getForm}/>
    </>
  );
};
