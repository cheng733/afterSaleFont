...
const ServiceInfo: React.FC = () => {
  ...
  const searchHandle = () => {
    ...
    Http.reqGetCustomer("/getCarMaintenAppoint", {
      name,
      offset: 0,
      size: 10,
    }).then((response: any) => {
  ...
  const onCreate = async (values: any) => {
    ...
    let status = "费用审核";
    await Http.reqAddMaintenDeatil("/insertMaintenDetailInfo", {
      faultAnaly,
      faultDesc,
      managePrice,
      name,
    ...
    rowId &&
      Http.reqEditStatusMaintenAppoint("/editStatusMaintenAppoint", {
        id: rowId,
        status: "费用审核",
      });
    setIsModalVisible(false);
